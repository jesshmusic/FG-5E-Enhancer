-- Since variable scopes are not cleanly handled in the original code I had to override everything
-- I have marked modified parts of code with Modified tag, you can search for it to see changes
...
function updatePowerWindowUses(nodeChar, w)
	...
	local bShow_override = false
	if string.find(sName, string.char(226)) or string.find(sName, string.char(183)) then  -- "●" or "·"
		bShow_override = true
	end
	--Modified </------------------------------------------------------------------------->
...
function updateUses()
	...
	--Modified <------------------------------------------------------------------------->
	local node = getDatabaseNode()
	if (Spell_Automation.SkipUsesUpdate == 0) or (OptionsManager.getOption("SA_DEL_HID") == "false") then
		powers.applyFilter();
	else
		Spell_Automation.SkipUsesUpdate = 0
	end
	--Modified </------------------------------------------------------------------------->
